...
public class ChangeModifyPromotionPercent extends Subject implements HandlerModifyPromotion {
    private HandlerModifyPromotion next;

    public ChangeModifyPromotionPercent(HandlerModifyPromotion next) {
        this.next = next;
    }
    ...
    public boolean doHandle(RequestModifyPromotion requestModifyPromotion) {
        try {
            ...
            SingletonCurrentPromotion.getInstance().getCurrentPromotion().setPromotionPercent(requestModifyPromotion.getPromotionPercent());
            notifyObserver();
